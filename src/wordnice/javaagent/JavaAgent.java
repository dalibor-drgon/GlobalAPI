...
public class JavaAgent {
	...
	protected static Instrumentation getRawInstrumentation() {
		...
		AccessController.doPrivileged(new PrivilegedAction<Object>() {
			public Object run() {
				...
				if(wordnice.utils.JavaUtils.isWindows()) {
					...
					File from = null;
					try {						
						File jdk = wordnice.utils.JavaUtils.getJDK();
						File jre = wordnice.utils.JavaUtils.getJRE();
						...
						if(!to.exists()) {
							wordnice.utils.FilesAPI.copyFile(to, from);
						}
					} catch(Throwable t2) {
					...
					LOG.info("Just little note: If you see errors from JavaAgent "
							...
							+ "native library in [" + wordnice.utils.JavaUtils.getJRE() + "/bin/]\n.");
					LOG.info(note);
				}
	...
	private static File findToolsJar() {
		File javaHomeFile = wordnice.utils.JavaUtils.getJDK();
